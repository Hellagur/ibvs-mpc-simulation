...
for k = 1:param.Tsteps
    textwaitbar(k,param.Tsteps,"CC-MPC simulation is running");
    ...
    % ------------------------------------------------------------
    % (4) Solve the chance-constrained MPC problem
    % Obtain the optimal control inputs and predicted trajectories
    % by minimizing the quadratic cost under probabilistic constraints.
    % ------------------------------------------------------------
    [xopt, uopt] = Opti_solver(mpc, param, hist, k, @Opti_con_define_ccmpc);    
...
%% Relative attitude trajectory (MRPs)
